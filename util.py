...
def get_prompt_template(prompt_type, filename="prompt_templates.json"):
    try:
        with open(filename , "r", encoding="utf-8") as file:
            templates = json.load(file)

        # Loop through the list to find the matching key
        for template in templates:
            if prompt_type in template:
                return template[prompt_type]

        return "Invalid type"  # If not found in the list
...
def get_vector_db_location() -> str:
   return os.getenv("VECTOR_DB_LOCATION")
